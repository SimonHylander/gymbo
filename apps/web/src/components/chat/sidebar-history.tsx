...
export function getChatHistoryPaginationKey(
  ...
  if (pageIndex === 0) {
    return `${API_BASE}/api/history?limit=${PAGE_SIZE}`;
  }
  ...
  return `${API_BASE}/api/history?ending_before=${firstChatFromPage.id}&limit=${PAGE_SIZE}`;
}

export function SidebarHistory({ user }: { user: MockUser | undefined }) {
  ...
  const handleDelete = () => {
    ...
    void fetch(
      `${API_BASE}/api/chat?id=${chatToDelete}`,
      { method: "DELETE" }
    );
