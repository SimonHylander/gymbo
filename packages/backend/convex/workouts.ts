  ...
  handler: async (ctx, { routineExternalId }) => {
    ...
    const routine = await ctx.db
      ...
      .withIndex("by_user_and_external_id", (q) =>
        q.eq("userId", userId).eq("externalId", routineExternalId)
      )
      .unique()
...
export const start = mutation({
  ...
  handler: async (ctx, { routineExternalId }) => {
    const userId = await requireUser(ctx)
    const routine = await getRoutineByExternalId(ctx, userId, routineExternalId)
...
export const setActiveExercise = mutation({
  ...
  handler: async (ctx, args) => {
    const userId = await requireUser(ctx)
    const workout = await getOwnedWorkout(ctx, userId, args.workoutId)
